...
Welcome to the Regex Training Center! 
...
def datepattern():
    dates = re.findall(r'(\d{4}(\/|\.|\-)\d{2}(\/|\.|\-)\d{2}|\b\d{2}(\/|\.|\-)\d{2}(\/|\.|\-)\d{4}|[A-Za-z]+\ \d{2}\,\ \d{4})',text)
    print(dates)

def emailpattern():
    email = re.findall(r'[A-Za-z0-9\.\-\_\+]+[\.]*+@[A-Za-z0-9\-]+\.[A-Za-z0-9\.]*(?<!\.)',text)
    print(email)
